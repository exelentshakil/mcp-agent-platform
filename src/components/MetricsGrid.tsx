...
// 4 Distinct Dope Wavy Sparkline Datasets for MCP Agent Platform Metrics
const mcpToolsDiscoveredTrend = [
  { t: 'S1', v: 6 },
  { t: 'S2', v: 12 },
  { t: 'S3', v: 18 },
  { t: 'S4', v: 22 },
...
const permissionEnforcementSla = [
  { t: 'Mon', v: 100 },
  { t: 'Tue', v: 100 },
  { t: 'Wed', v: 100 },
  { t: 'Thu', v: 100 },
...
const telemetryStream = [
  ...
  { time: '10:00', ops: 4790, latency: 14.1, stage: 'MCP Schema Discovery' },
  ...
  { time: '14:00', ops: 6450, latency: 13.2, stage: 'Agent Tool Execution Gate' },
...
export function MetricsGrid() {
  ...
  const renderCardChart = (idx: number) => {
    ...
    if (idx === 0) {
      // Card 0: MCP Tools Discovery Trend (Stripe Blurple wavy gradient area)
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={mcpToolsDiscoveredTrend} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="rounded-[4px] bg-[var(--color-surface)] border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium shadow-xs text-[var(--color-text-primary)]">
                      <span className="font-bold text-[#533AFD] dark:text-[#7A68FF]">${payload[0].value}</span> Tools Active
                    </div>
                  );
                }
                return null;
    ...
    if (idx === 1) {
      ...
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={agentInvocationsTrend} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
            ...
            <Tooltip
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
                  return (
                    <div className="rounded-[4px] bg-[var(--color-surface)] border border-[var(--color-border)] px-2.5 py-1 text-xs font-medium shadow-xs text-[var(--color-text-primary)]">
                      <span className="font-bold text-[#057A55] dark:text-emerald-400">${payload[0].value}k</span> Tool Invocations
                    </div>
                  );
                }
                return null;
    ...
    if (idx === 2) {
      ...
      return (
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={permissionEnforcementSla} margin={{ top: 2, right: 2, left: 2, bottom: 0 }}>
            <defs>
